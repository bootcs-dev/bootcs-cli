...
def start_device_flow() -> DeviceCodeResponse:
    ...
    try:
        ...
        raise DeviceFlowError(code="NETWORK_ERROR", message=f"Network error: {e}")


def poll_for_token(device_code: str, interval: int = 5, timeout: int = 300) -> TokenResponse:
    """
    ...
    while time.time() - start_time < timeout:
        ...
        try:
            response = requests.post(url, json={"device_code": device_code}, timeout=30)
            data = response.json()
            ...
            if error_code:
                ...
                if error_code == "authorization_pending":
                    ...
                    raise DeviceFlowError(
                        code="expired_token", message="Device code has expired. Please try again."
                    )
                elif error_code == "access_denied":
                    raise DeviceFlowError(code="access_denied", message="Authorization was denied.")
                else:
                    raise DeviceFlowError(
            ...
            if response.ok:
                ...
                if token:
                    return TokenResponse(token=token, user=payload.get("user", {}))

        except requests.RequestException:
            # Network error, retry
            continue

    raise DeviceFlowError(code="timeout", message="Authentication timed out. Please try again.")
