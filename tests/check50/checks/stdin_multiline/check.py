...
def prints_hello_name_non_chaining():
    ...
    check50.run("python3 foo.py").stdin("bar\nbaz", prompt=False).stdout("hello bar").stdout(
        "hello baz"
    )
...
def prints_hello_name_chaining():
    ...
    check50.run("python3 foo.py").stdin("bar", prompt=False).stdout("hello bar").stdin(
        "baz", prompt=False
    ).stdout("hello baz")
...
def prints_hello_name_chaining_order():
    ...
    check50.run("python3 foo.py").stdin("bar", prompt=False).stdin("baz", prompt=False).stdout(
        "hello bar"
    ).stdout("hello baz")
